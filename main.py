...
def unzipping() -> None:
    ...
    # Unzip idbm archive
    try:
        if not os.path.exists(paths_dict['output_imdb_file'] + paths_dict['check_name_file_imdb']):
        ...
        if not os.path.exists(paths_dict['output_wiki_file'] + paths_dict['name_file_wiki']):
...
def generate_dataframe() -> pd.DataFrame:
    ...
    df_metadata__ = df_metadata[['id', 'budget', 'original_title']].rename(columns = {"id" : "tmdbId"})
    df_metadata__['tmdbId'] = pd.to_numeric(df_metadata__['tmdbId'], errors='coerce') # coerce -> invalid data will be set to NaN
    df_metadata__['budget'] = pd.to_numeric(df_metadata__['budget'], errors='coerce') # coerce -> invalid data will be set to NaN
    ...
    df_wiki_urls = load_wiki_dataframe(xml_path = paths_dict['output_wiki_file'] + paths_dict['name_file_wiki'])
...
def main():
    ...
    client_psql = client_postgreSQL.ClientPostgreSQL(username = 'postgres', password = 'abcd1234', database = 'mattia')
...
class CommandLine:
    ...
    def __init__(self):
        ...
        # With store i must take something after --debug, with store_true i can pass --debug only
        # print(options.debug) return a dict, but options cannot be accessed as a dict
        parser.add_option('-q', '--debug', action="store_true", dest="debug", help="debug parameters", default=False)
